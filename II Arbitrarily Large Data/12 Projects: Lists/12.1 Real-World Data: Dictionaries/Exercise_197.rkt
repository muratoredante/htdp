...
#reader(lib "htdp-beginner-abbr-reader.ss" "lang")((modname Exercise_197) (read-case-sensitive #t) (teachpacks ((lib "image.rkt" "teachpack" "2htdp") (lib "batch-io.rkt" "teachpack" "2htdp"))) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ((lib "image.rkt" "teachpack" "2htdp") (lib "batch-io.rkt" "teachpack" "2htdp")) #f)))
; Picking out the maximum pair is more efficient
; Having the sorted list is more flexible to show top n
...
; List -> Posn
(check-expect (max-element (list (make-posn "a" 3) (make-posn "b" 1) (make-posn "c" 1)))
              (make-posn "a" 3))
(check-expect (max-element (list (make-posn "a" 3) (make-posn "b" 5) (make-posn "c" 1)))
              (make-posn "b" 5))
(check-expect (max-element (list (make-posn "a" 3) (make-posn "b" 5) (make-posn "c" 7)))
              (make-posn "c" 7))
(define (max-element l)
  (aux-max l (make-posn "0" 0)))


(define (aux-max l cm)
    (cond
    [(empty? l) cm]
    [else (if (> (posn-y (first l)) (posn-y cm))
              (aux-max (rest l) (first l))
              (aux-max (rest l) cm))]))

; Dictionary -> Posn
(define (most-frequent d)
  (max-element (count-by-letter d)))
